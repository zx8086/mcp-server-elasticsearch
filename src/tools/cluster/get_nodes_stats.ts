...
const nodesStatsSchema = {
  ...
  properties: {
    ...
    metric: {
      type: "string",
      description: "CRITICAL: Specify exact metrics needed. Options: 'os', 'jvm', 'fs', 'process', 'http', 'transport', 'indices'. Combine: 'os,jvm'"
    },
    indexMetric: {
      type: "string",
...
function createNodesStatsMcpError(
  ...
  return new McpError(
    ...
    `[elasticsearch_get_nodes_stats] ${message}`,
    context.details
  );
}
