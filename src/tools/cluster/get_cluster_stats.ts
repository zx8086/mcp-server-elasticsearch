...
const clusterStatsSchema = {
  ...
  properties: {
    nodeId: {
      type: "string",
      description: "A comma-separated list of node IDs or names to limit returned information. Leave empty for all nodes."
    },
    timeout: {
      type: "string",
...
function createClusterStatsMcpError(
  ...
  return new McpError(
    ...
    `[elasticsearch_get_cluster_stats] ${message}`,
    context.details
  );
}
