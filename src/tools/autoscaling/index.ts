...
export class AutoscalingError extends Error {
  constructor(message: string, public readonly policyName?: string) {
    super(message);
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes('not found')) {
        throw new McpError(
          ...
          `Autoscaling policy not found: ${args.name}`
        );
      }
...
const putPolicyImpl = async (client: Client, args: z.infer<typeof putPolicySchema>) => {
  try {
    ...
    if (error instanceof Error && error.message.includes('validation')) {
      throw new McpError(
        ErrorCode.InvalidRequest,
        `Invalid autoscaling policy configuration: ${error.message}`
      );
    }
...
export const putPolicy = {
  ...
  description: 'Create or update an autoscaling policy in Elasticsearch. Best for capacity management, resource automation, cluster scaling. Use when you need to define autoscaling policies for Elasticsearch Service, ECE, or ECK environments. NOTE: Designed for indirect use.',
