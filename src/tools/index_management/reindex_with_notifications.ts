    ...
    try {
      ...
      try {
        ...
        if (!sourceExists) {
          ...
          throw new McpError(ErrorCode.InvalidParams, `Source index ${params.source.index} does not exist`);
        }
        ...
        await notificationManager.sendInfo(`Source index validated: ${totalDocs} documents found`, {
          source_index: params.source.index,
      ...
      try {
        ...
        if (destExists) {
          ...
          await notificationManager.sendInfo(`Destination index ${params.dest.index} will be created`, {
            dest_index: params.dest.index,
            phase: "preparation",
          });
        }
      ...
      if (params.wait_for_completion) {
        ...
        await notificationManager.sendInfo("Reindex operation started (synchronous mode)", {
          mode: "synchronous",
          requests_per_second: params.requests_per_second,
          phase: "execution",
        });
        ...
        const progressInterval = setInterval(async () => {
          ...
          await tracker.updateProgress(Math.min(currentProgress, 90), "Reindexing documents...");
        }, 2000);
        ...
        if (reindexResult.failures && reindexResult.failures.length > 0) {
          await tracker.complete(result, `Reindex completed with ${reindexResult.failures.length} failures`);
          ...
          await tracker.complete(result, `Reindex completed successfully: ${reindexResult.total} documents processed`);
        ...
        await notificationManager.sendInfo("Reindex operation started (asynchronous mode)", {
          mode: "asynchronous",
          requests_per_second: params.requests_per_second,
          phase: "execution",
        });
        ...
        await tracker.complete({ task: reindexResult.task }, `Reindex task created: ${reindexResult.task}`);

        await notificationManager.sendInfo(`Reindex task created: ${reindexResult.task}`, {
