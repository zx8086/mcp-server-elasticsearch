...
const updateAliasesSchema = {
  ...
  properties: {
    actions: {
      type: "array",
      description: "Array of alias actions to perform atomically. Each action should have 'add', 'remove', or 'remove_index' key with appropriate configuration",
      items: {
        type: "object",
        description: "Alias action: {add: {index: 'idx', alias: 'alias'}}, {remove: {index: 'idx', alias: 'alias'}}, or {remove_index: {index: 'idx'}}"
      }
    },
    timeout: {
      type: "string",
...
function validateAliasActions(actions: any[]): void {
  ...
  for (let i = 0; i < actions.length; i++) {
    ...
    if (!validActionTypes.includes(actionType)) {
      throw new Error(`Action ${i}: Invalid action type '${actionType}'. Must be one of: ${validActionTypes.join(', ')}`);
    }
...
export const registerUpdateAliasesTool: ToolRegistrationFunction = (server: McpServer, esClient: Client) => {
  ...
  const updateAliasesHandler = async (args: any): Promise<SearchResult> => {
    ...
    try {
      ...
      const actionSummary = params.actions.reduce((summary, action) => {
      ...
      if (error instanceof Error && error.message.includes('index_not_found_exception')) {
        throw createMcpError(`One or more indices in the actions do not exist. Check index names and ensure they exist before updating aliases.`, {
          toolName: 'elasticsearch_update_aliases',
          type: 'invalid_alias',
          details: { actions: args.actions }
        });
      ...
      if (error instanceof Error && error.message.includes('alias_not_found_exception')) {
        throw createMcpError(`One or more aliases in remove actions do not exist. Check alias names before attempting to remove.`, {
          toolName: 'elasticsearch_update_aliases',
      ...
      if (error instanceof Error && error.message.includes('invalid_alias_name_exception')) {
        throw createMcpError(`One or more alias names are invalid. Check alias name formats and ensure they don't conflict with existing indices.`, {
          toolName: 'elasticsearch_update_aliases',
          type: 'invalid_alias',
          details: { actions: args.actions }
        });
