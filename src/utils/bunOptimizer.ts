...
export class BunFileManager {
   ...
   */
  static async processLargeResponse(
    data: any, 
...
export class BunPerformanceTimer {
  ...
  start(): void {
    ...
    const endTime = BunRuntimeDetection.isBun() 
      ? Bun.nanoseconds()
  ...
  static async measure<T>(
    ...
    const start = BunRuntimeDetection.isBun() 
      ? Bun.nanoseconds()
      : performance.now() * 1_000_000;
      
    const result = await fn();
    
    const end = BunRuntimeDetection.isBun() 
      ? Bun.nanoseconds()
      : performance.now() * 1_000_000;
