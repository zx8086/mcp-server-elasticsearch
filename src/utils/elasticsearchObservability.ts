...
import { withNestedTrace, type TraceMetadata } from "./tracing.js";
...
export class ObservableTransport extends Transport {
  ...
  private updateAggregateMetrics(): void {
    ...
    const recentRequests = this.metricsWindow.filter(
      req => (req.endTime || req.startTime) > fiveMinutesAgo
    );

    if (recentRequests.length > 0) {
      // Calculate average response time
      const durations = recentRequests.filter(req => req.duration).map(req => req.duration!);
      this.connectionMetrics.averageResponseTime = durations.length > 0 
  ...
  public getRecentRequests(minutes: number = 5): ElasticsearchRequestMetrics[] {
    const cutoff = Date.now() - (minutes * 60 * 1000);
    return this.metricsWindow.filter(req => 
      (req.endTime || req.startTime) > cutoff
    );
  }
