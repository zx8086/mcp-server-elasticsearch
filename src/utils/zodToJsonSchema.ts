...
export function zodToJsonSchemaCompat(schema: z.ZodTypeAny, options: any = {}): any {
  ...
  if (typeof (z as any).toJSONSchema === 'function') {
    ...
    try {
      ...
      if (error.message && (error.message.includes("Transforms cannot be represented") || 
    ...
    if (jsonSchema.type === "object" && jsonSchema.required && jsonSchema.properties) {
      ...
      if (shape) {
        for (const [key, field] of Object.entries(shape)) {
          ...
          const isOptional = typeof (field as any).isOptional === 'function' ? 
            (field as any).isOptional() : false;
    ...
    if (jsonSchema.type === "object") {
      ...
      if (jsonSchema.additionalProperties && typeof jsonSchema.additionalProperties === "object" && Object.keys(jsonSchema.additionalProperties).length === 0) {
        ...
        for (const [key, prop] of Object.entries(jsonSchema.properties as any)) {
          if (prop && typeof prop === "object" && prop.type === "object") {
            ...
            if (shape) {
              ...
              } else if (prop.additionalProperties && typeof prop.additionalProperties === "object" && Object.keys(prop.additionalProperties).length === 0) {
                prop.additionalProperties = true;
              }
            }
          }
  ...
  // If we reach here, we're likely on an unsupported Zod version
  throw new Error(
    "Schema conversion failed. This project requires Zod 4.x with native toJSONSchema support."
  );
}
