...
export class IntelligentCache {
  ...
  private recordAccess(key: string, _hit: boolean, toolName?: string): void {
    ...
    if (pattern.accessCount > 1) {
      ...
      pattern.averageAccessInterval = pattern.averageAccessInterval * 0.8 + timeSinceLastAccess * 0.2;
  ...
  private calculateUsageScore(pattern: CacheUsagePattern, toolName?: string): number {
    ...
    const predictability =
      pattern.averageAccessInterval > 0
        ? Math.max(0, 1 - Math.abs(now - pattern.predictedNextAccess) / pattern.averageAccessInterval)
        : 0;
  ...
  private schedulePrefetchAnalysis(key: string, _toolName?: string): void {
  ...
  private async performPrefetchAnalysis(): Promise<void> {
    try {
      ...
      for (const [key, pattern] of this.usagePatterns) {
        if (
          pattern.score > this.options.prefetchThreshold &&
          pattern.predictedNextAccess > 0 &&
          pattern.predictedNextAccess <= now + 5 * 60 * 1000
        ) {
  ...
  private handleCacheEviction(key: string, _value: any): void {
    if (metrics.isEnabled()) {
