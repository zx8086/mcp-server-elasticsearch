...
describe('End-to-End Integration Tests', () => {
  ...
  test('should integrate monitoring components', async () => {
    try {
      ...
      metrics.updateCacheHitRatio(0.85);
  ...
  test('should integrate security audit system', async () => {
    try {
      const { SecurityAuditTrail, SecurityEventType } = await import('../../src/security/auditTrail.js');
      ...
      expect(insights.topUsers).toContain('test-user');

      console.log(`✅ Security audit integration: ${insights.totalEvents} events logged`);
