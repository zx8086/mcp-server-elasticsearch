...
import { createMockServer, createMockClient } from "./utils/test-helpers.js";
...
describe("Zod 3.x Compatibility - Critical Fix Validation", () => {
  ...
  describe("MCP Protocol Metadata Handling", () => {
    test("should filter out MCP metadata and extract actual parameters", async () => {
      ...
      // Should only receive the actual parameter, not metadata
      expect(receivedParams.data).toBe("test-value");
      expect(receivedParams.signal).toBeUndefined();
      expect(receivedParams.requestId).toBeUndefined();
      expect(receivedParams.sessionId).toBeUndefined();
    });
  });

  describe("Validation Error Messages", () => {
    test("should provide clear validation errors for wrong types", async () => {
      wrappedServer.tool(
        "test_validation",
        "Test validation",
        {
          number: z.number().describe("Must be a number"),
          string: z.string().describe("Must be a string")
        },
        async () => {
          return { content: [{ type: "text", text: "success" }] };
        }
      ...
      const tool = (server as any).getTool("test_validation");
      ...
      try {
        ...
        // The error should mention both fields
