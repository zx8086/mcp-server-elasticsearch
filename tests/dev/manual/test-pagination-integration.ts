  ...
  try {
    ...
    if (ilmTool) {
      try {
        ...
        if (text.includes('of ') && text.includes('⚠️')) {
          console.log('   ✅ Result contains proper pagination metadata');
        } else {
          console.log('   ⚠️  Result may not contain pagination info (could be less than limit)');
        }
    ...
    console.log('   ✓ Tools respect limit parameters');
    console.log('   ✓ Default limits are applied when no limit specified');
    console.log('   ✓ Consistent pagination headers across tools');
