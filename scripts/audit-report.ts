...
import { AuditEventType, SecurityAuditTrail } from "../src/security/auditTrail.js";
...
async function generateSampleAuditData(auditTrail: SecurityAuditTrail): Promise<void> {
  ...
  await auditTrail.logSuspiciousActivity("Multiple failed authentication attempts from same IP", "high", {
...
function calculateSecurityScore(summary: any): number {
  ...
  const failureEvents = (summary.eventsByType.authentication_failure || 0) + (summary.eventsByType.error_event || 0);
  if (failureEvents > 5) {
