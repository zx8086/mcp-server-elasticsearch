...
async function runBenchmarks() {
  ...
  const benchmarkConfig: BenchmarkConfig = {
    ...
    iterations: process.env.BENCHMARK_ITERATIONS ? parseInt(process.env.BENCHMARK_ITERATIONS) : 100,
    warmup: 10,
  };
  ...
  for (const toolName of benchmarkConfig.tools) {
    ...
    try {
      ...
      console.log(`   ✅ Completed: avg ${benchmark.summary.avgDuration.toFixed(2)}ms, ` +
