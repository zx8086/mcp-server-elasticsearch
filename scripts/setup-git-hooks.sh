...
if ! command -v bun &> /dev/null; then
    echo "[FAIL] Bun is not installed. Please install bun first."
    exit 1
fi
...
if [ -f "package.json" ] && grep -q '"lint"' package.json; then
    ...
    bun run lint
    if [ $? -ne 0 ]; then
        echo "[FAIL] Linting failed. Please fix the issues before committing."
        exit 1
    fi
fi
...
echo "[PASS] Git pre-commit hook installed successfully!"
