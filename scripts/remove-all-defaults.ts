    ...
    if (file.isDirectory()) {
      ...
      for (const pattern of patterns) {
        ...
        for (const match of matches.reverse()) {
          ...
          const afterContext = modified.substring(
            match.index! + fullMatch.length,
            Math.min(modified.length, match.index! + fullMatch.length + 100),
          );

          // Look for Zod indicators
          if (
            beforeContext.includes("z.") ||
            afterContext.includes(".describe(") ||
            afterContext.includes(".optional()")
          ) {
