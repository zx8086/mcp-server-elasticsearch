...
class ElasticsearchTestDataGenerator implements TestDataGenerator {
  ...
  async generateMockElasticsearchData(): Promise<void> {
    ...
    await writeFile(
      path.join(this.outputDir, 'elasticsearch', 'mappings.json'),
  ...
  async generateCacheTestData(): Promise<void> {
    ...
    const cacheScenarios = {
      access_patterns: [
        ...
        },
        {
          ...
          ttl_seconds: 3600,
          sample_keys: [
            'mappings:logs-2024.01',
            'mappings:metrics-2024.01',
            'mappings:user-data'
          ],
          access_count: 20,
    ...
    await writeFile(
      path.join(this.outputDir, 'cache', 'test-scenarios.json'),
      JSON.stringify(cacheScenarios, null, 2)
    );
    ...
    await writeFile(
      path.join(this.outputDir, 'cache', 'sample-data.json'),
      JSON.stringify(sampleCacheData, null, 2)
    );
  ...
  async generateAll(): Promise<void> {
    ...
    await writeFile(
      path.join(this.outputDir, 'README.json'),
