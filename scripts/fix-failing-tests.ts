...
const fixes: TestFix[] = [
  ...
  // Fix elasticsearch_search tests missing required params
  {
    ...
    pattern: /const result = await registeredHandler\({\s*index: "test-index",\s*queryBody: {\s*query: { match: { title: "test" } }\s*},?\s*}\);/g,
    replacement: 'const result = await registeredHandler({\n      index: "test-index",\n      queryBody: {\n        query: { match: { title: "test" } },\n      },\n    });',
  },
  ...
  // Fix core tools tests
  {
    ...
    pattern: /registerListIndicesTool\(mockServer as any, mockClient\);\s*const result = await \(mockServer as any\)\.getTool\("elasticsearch_list_indices"\)\.handler\({}\);/g,
    replacement: 'registerListIndicesTool(mockServer as any, mockClient);\n    const result = await (mockServer as any).getTool("elasticsearch_list_indices").handler({ indexPattern: "*" });',
  },
  ...
  {
    file: "tests/tool-improvements.test.ts",
    pattern: /\/\/ Should have executed with default parameters\s*expect\(executedWithArgs\)\.toBeDefined\(\);\s*expect\(executedWithArgs\.index\)\.toBe\("\*"\);/g,
    replacement: '// Should have executed with provided parameters\n      expect(executedWithArgs).toBeDefined();\n      expect(executedWithArgs.index).toBe("*");',
  },
  
  // Update test to pass index param
  {
    file: "tests/tool-improvements.test.ts",
    pattern: /const tool = \(mockServer as any\)\.getTool\("elasticsearch_get_index"\);\s*const result = await tool\.handler\({}\);/g,
    replacement: 'const tool = (mockServer as any).getTool("elasticsearch_get_index");\n      const result = await tool.handler({ index: "*" });',
  },
  
  // Fix comprehensive tools tests
  {
    file: "tests/tools/comprehensive-tools.test.ts",
    pattern: /const result = await \(mockServer as any\)\.getTool\("elasticsearch_get_index_template"\)\.handler\({}\);/g,
    replacement: 'const result = await (mockServer as any).getTool("elasticsearch_get_index_template").handler({ name: "*" });',
  },
  
  {
    file: "tests/tools/comprehensive-tools.test.ts",
    pattern: /const result = await \(mockServer as any\)\.getTool\("elasticsearch_get_aliases"\)\.handler\({}\);/g,
    replacement: 'const result = await (mockServer as any).getTool("elasticsearch_get_aliases").handler({ index: "*" });',
  },
  
  {
    file: "tests/tools/comprehensive-tools.test.ts",
    pattern: /const result = await \(mockServer as any\)\.getTool\("elasticsearch_enrich_get_policy"\)\.handler\({}\);/g,
    replacement: 'const result = await (mockServer as any).getTool("elasticsearch_enrich_get_policy").handler({ name: "*" });',
  },
];
